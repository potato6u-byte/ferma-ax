...
def init_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS batch_master (
            batch_id TEXT PRIMARY KEY, product_name TEXT, target_tank TEXT,
            worker_name TEXT, start_datetime TEXT, start_time_korean TEXT,
            outdoor_temp REAL, indoor_temp REAL, status TEXT
        )
    ''')
    c.execute('''
        CREATE TABLE IF NOT EXISTS step_temperature_logs (
            id INTEGER PRIMARY KEY AUTOINCREMENT, batch_id TEXT, step_code TEXT,
            step_name TEXT, log_time TEXT, step_duration_str TEXT, scada_tag TEXT,
            ai_recommended_temp REAL, operator_set_temp REAL, actual_measured_temp REAL,
            action_status TEXT, FOREIGN KEY (batch_id) REFERENCES batch_master (batch_id)
        )
    ''')
    c.execute('''
        CREATE TABLE IF NOT EXISTS batch_mqi_final (
            batch_id TEXT PRIMARY KEY, completed_at TEXT, actual_duration INTEGER,
            final_acidity REAL, final_ph REAL, viscosity_cp REAL, syneresis_rate REAL,
            taste_score REAL, mqi_total_score REAL, worker_memo TEXT,
...
if hasattr(st, "dialog"):
    ...
    def step_indoor_temp_modal(s_code, s_name):
        ...
        if st.button("온도 확정 및 공정 시작", type="primary", use_container_width=True):
            ...
            st.session_state.calc_res = calculate_optimal_temperatures(calc_h, pop_in_val, curr_t, min_t, max_t)
            st.rerun()
        ...
        with st.form("quick_indoor_temp_form", clear_on_submit=False):
            ...
            with c_in_btn:
                ...
                if submitted_lock:
                    ...
                    st.session_state.temp_locked = True
                    calc_h = st.session_state.get("start_hour", round(float(kst_now.hour + kst_now.minute / 60.0), 1))
                    st.session_state.calc_res = calculate_optimal_temperatures(calc_h, typed_in_temp, curr_t, min_t, max_t)
                    st.rerun()
...
if st.session_state.process_step == 0:
    ...
    if st.button("이 설정으로 [배치 작업 시작 (A100 착수)]", type="primary", use_container_width=True):
        if not worker_name.strip() or not product_name_input.strip():
            st.error("품목명과 작업자명을 모두 입력바람.")
        else:
            ...
            st.session_state.target_tank = "A300 발효탱크"
            st.session_state.worker_name = worker_name.strip()
            st.session_state.start_hour = round(float(exact_kst_dt.hour + exact_kst_dt.minute / 60.0), 1)
            ...
            st.session_state.calc_res = calculate_optimal_temperatures(st.session_state.start_hour, st.session_state.indoor_t, curr_t, min_t, max_t)
            save_batch_start(batch_id_gen, product_name_input.strip(), "A300 발효탱크", worker_name.strip(), exact_kst_dt.strftime("%Y-%m-%d %H:%M:%S"), start_korean_val, curr_t, st.session_state.indoor_t)
    ...
    st.info(f"**AI 추천 살균냉각온도:** **{c_res['rec_cooling']} ℃**")
    ...
    if st.button("A200 확인 [다음 A300 이동]", type="primary", use_container_width=True):
        ...
        log_step_temp(st.session_state.batch_id, "A200", "살균냉각투입", step_dur_str, "P101TC02", c_res['rec_cooling'], op_set_cool, act_meas_cool, "정상 설정")
    ...
    if st.button("5H 점검 기록 [다음 A400 이동]", type="primary", use_container_width=True):
        ...
        log_step_temp(st.session_state.batch_id, "A300", "발효재킷보온", step_dur_str, "PHE301WCV01", c_res['rec_hotwater'], op_set_hw, act_meas_5h, "정상 궤적")
    ...
    st.subheader(f"[Step 5: A500] {st.session_state.product_name} 시럽 살균/냉각")
    ...
    st.info(f"**AI 선행 냉각 권장 시점:** 접종 후 **{trig_min}분**")
...
# =============================================================
    ...
    if st.button("전체 공정 최종 종결 [엑셀 자동 누적 & 구글 시트 전송]", type="primary", use_container_width=True):
        ...
        gsheet_payload = {
            ...
            "cooling_temp": final_record["살균냉각(℃)"], "hotwater_temp": final_record["핫워터(℃)"],
            "phe_temp": final_record["급속칠링(℃)"], "duration": final_record["소요시간(분)"],
            "acidity": final_record["산도"], "ph": final_record["pH"], "viscosity": final_record["점도(cP)"],
            "syneresis": final_record["유청분리(%)"], "taste": final_record["맛점수"],
            "mqi_score": final_record["MQI점수"], "memo": final_record["특이사항"]
        }
