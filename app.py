# -*- coding: utf-8 -*-
"""
FermaAX™ Full-Process SCADA SOP & AI Temperature Controller v5.1
생산품목: 런 발효유 (단일 품목) 전용 SCADA 순차 공정 제어 & AI 최적 처방 시스템
"""
...
# 1. 반응형 페이지 설정
st.set_page_config(
    page_title="FermaAX - 런 발효유 SCADA 공정 제어기",
    page_icon="🧪",
    layout="wide",
...
# DB 헬퍼 함수
def save_batch_start(batch_id, p_name, tank, worker, s_dt, out_t, in_t):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute('''
        INSERT OR REPLACE INTO batch_master VALUES (?, ?, ?, ?, ?, ?, ?, 'RUNNING')
    ''', (batch_id, p_name, tank, worker, s_dt, out_t, in_t))
    conn.commit()
    conn.close()
...
def get_all_completed_batches():
    conn = sqlite3.connect(DB_FILE)
    query = '''
        SELECT b.batch_id, b.product_name, b.target_tank, b.worker_name, b.start_datetime,
...
st.title("🧪 FermaAX™ [런 발효유] SCADA 단계별 공정 제어기")
st.caption(f"생산품목: **런 발효유** | 부산 기장군 기상 연동 (외기 {curr_t}℃ | 일교차 {min_t}℃ ~ {max_t}℃) | SCADA P&ID 순차 제어")
...
# =============================================================
# STEP 0: 배치 착수 등록 (런 발효유 단일 품목 고정)
# =============================================================
if st.session_state.process_step == 0:
    ...
    with col1:
        st.text_input("생산 품목", value="런 발효유", disabled=True)
        product_name = "런 발효유"
    ...
    with col3:
        ...
        batch_id_gen = f"{now_dt.strftime('%Y%m%d')}_RUN_{target_tank[:3]}_{int(start_hour_val):02d}H"
        st.info(f"생성될 배치 ID: **{batch_id_gen}**")

    st.divider()
    if st.button("🚀 이 설정으로 [런 발효유 배치 시작 (A100 착수)]", type="primary", use_container_width=True):
        if not worker_name:
            ...
            save_batch_start(
                batch_id_gen, product_name, target_tank, worker_name,
...
# =============================================================
    ...
    st.subheader(f"📍 [Step 1: A100] 런 발효유 Base Mix 배합 단계 (배치: {st.session_state.batch_id})")
    
    st.markdown("""
    * **담당 구역:** 101~104호 베이스 배합탱크 및 분말 믹서 (`MX101`, 45.0 Hz)
    * **주요 작업:** 런 발효유 원유 및 배합원료 계량 투입, 배합 탱크 교반 가동, 살균 라인 이송 밸브 점검
...
# =============================================================
    ...
    st.subheader(f"📍 [Step 2: A200] 런 발효유 살균 및 냉각 투입 단계 (배치: {st.session_state.batch_id})")
...
# =============================================================
    ...
    st.subheader(f"📍 [Step 3: A300] 런 발효유 발효 및 재킷 보온 제어 (배치: {st.session_state.batch_id})")
...
# =============================================================
    ...
    st.subheader(f"📍 [Step 4: A400/A500] 런 발효유 시럽 배합 및 살균 (배치: {st.session_state.batch_id})")
    
    st.markdown("""
    * **담당 구역:** 401~402호 시럽 배합탱크 (A400) 및 시럽 전용 살균기 (A500)
    * **주요 작업:** 런 발효유 전용 시럽 용해, 살균 온도(`P201TC02`, 85℃ 살균) 및 농도 전도도(`P2B1CT02`) 점검
...
# =============================================================
    ...
    st.subheader(f"📍 [Step 5: A600] 런 발효유 블렌딩 및 PHE301 급속 냉각 (배치: {st.session_state.batch_id})")
    st.warning("⚠️ **[핵심 제어점 3: 선행 냉각 트리거]** 런 발효유 후산도(+0.035)를 고려한 AI 선행 냉각 시점을 확인바람.")
...
# =============================================================
# STEP 6: A700 공정 (서지탱크/충전 & 런 발효유 다차원 MQI 품질 검증)
# =============================================================
elif st.session_state.process_step == 6:
    st.subheader(f"📍 [Step 6: A700] 런 발효유 서지탱크 이송 & 다차원 품질검증 (배치: {st.session_state.batch_id})")
    st.info("💡 런 발효유 최종 완제품의 산도·pH 및 텍스처(점도, 유청분리), 관능(맛) 점수를 종합 평가하여 DB에 영구 기록함.")
    ...
    with st.form("a700_mqi_form"):
        ...
        g1, g2 = st.columns(2)
        with g1:
            f_taste = st.slider("관능 평가 점수 (런 발효유 맛 밸런스, 5점 만점)", 1.0, 5.0, 4.8, 0.1)
        with g2:
            f_memo = st.text_input("작업 특이사항 메모", placeholder="예: 런 발효유 골든 기준 완벽 수렴, 풍미 및 바디감 우수.")
        ...
        st.markdown(f"### 🏆 런 발효유 종합 품질점수 (MQI Score): **`{mqi_total} / 100 점`**")
...
# =============================================================
    ...
    st.success(f"🎉 런 발효유 배치 [{st.session_state.batch_id}] 공정이 성공적으로 종결되었으며 모든 단계별 온도 이력이 기록되었음!")
    ...
    with tab_r2:
        ...
        if not all_df.empty:
            ...
            st.download_button(
                label="📥 런 발효유 누적 MQI 품질 이력 CSV 다운로드",
                data=csv_export,
                file_name=f"RUN_FermaAX_MQI_{date.today().strftime('%Y%m%d')}.csv",
                mime="text/csv",
                use_container_width=True
            )

    st.divider()
    if st.button("🔄 새로운 런 발효유 배치 작업 시작하기", type="primary", use_container_width=True):
        st.session_state.process_step = 0
        st.session_state.batch_id = ""
        st.rerun()
